...
def exe():
    ...
    try:
        results = root.find_all(type, limit = a)
    except:
    ...
    if len(results) == 0:#無法進行資料抓取
        return flask.render_template('page.html', show_type_area = "抓取的類型為: " + type, show_url_area = "選擇網址為: " + url, show_num_area = '下載數量為: ' + num, show_status_area = "很抱歉，此網站不允許抓取該資料，請另外選則的網站。")
    if type == "img": #將圖片存到到images中，並且每次下載會創建新資料夾
        ...
        for index, link in enumerate(img_link):
            ...
            with open("images/" + new_folder +"/" + str(index+1) + ".jpg", "wb") as file:  # 開啟資料夾及命名圖片檔
        ...
        with open("documents/" + new_txt + ".txt", "a") as file:  # 開啟資料夾及命名圖片檔
            for line in text:
                file.write( line + '\n')
    return flask.render_template('page.html', show_type_area = "抓取的類型為: " + type, show_url_area = "選擇網址為: " + url, show_num_area = '下載數量為: ' + num, show_status_area = "共找到 " + str(len(results)) + " 筆資料，開始進行下載。")
